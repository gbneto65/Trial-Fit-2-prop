...
def power_prop( prop1, prop2, sample_size, p_value, side):

        if prop1<prop2 and side==1:
           ha='smaller'
        elif prop1>prop2 and side==1:
           ha='larger' 
        else:
           ha='two-sided'
        diff_prop = abs(prop2-prop1)
        #print(type(prop1), prop2, type(sample_size), type(p_value),side, ha)
        
        power_prop = power_proportions_2indep(
            diff=diff_prop,
            prop2 = prop2,
            nobs1 = sample_size,
            ratio=1,
        ...
        return round(power_prop,2) # return power
...
#    def __init__(self, p1, p2, z_alfa = 1.96 ,z_beta = 0.84):
     def __init__(self, p1, p2, p_z_alfa, p_z_beta, side):
